...
class JobStore:
    ...
    def create(self, prompt, files):
        ...
        status = {"id": job_id, "status": "queued", "prompt": prompt, "bundle_id": job_id}
    ...
    def run(self, job_id, generator_dir):
        ...
        try:
            command = [sys.executable, "agent.py", "--input", str(source_root), "--prompt", status["prompt"], "--output", str(output)]
...
class TelemetryStore:
    ...
    def analytics(self, bundle_id):
        ...
        return {
            "views": len(sessions), "completed_sessions": len({item["session_id"] for item in completion}),
            "fast_completion_count": sum(item.get("needs_review") is True for item in completion),
            "checkpoints": rows,
